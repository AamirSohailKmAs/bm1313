<x-admin-layout>
  <form class="p-5 border bg-slate-300 rounded-lg mb-2 border-gray-400 w-full" action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    <div class="flex flex-col md:flex-row">
      <div class="mb-4 w-full">
        <x-label.default for="name">{{ __('Name') }}</x-label.default>
        <x-input.default id="name" name="name" value="{{ $user->name }}" placeholder="{{ __('Enter Name') }}" required />
      </div>
      ...
      <div class="mb-4 mx-2 w-full">
        ...
        <input class="shadow appearance-none border rounded w-full py-2 px-3 bg-gray-100 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" name="username" type="text" value="{{ $user->username }}" placeholder="{{ __('Enter Username') }}" required>
      </div>
      ...
      <div class="mb-4 mx-2 w-full">
        ...
        <input class="shadow appearance-none border rounded w-full py-2 px-3 bg-gray-100 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="password" name="password" type="text" placeholder="{{ __('Enter Password') }}">
      </div>
    ...
    <div class="flex flex-col md:flex-row">
      <div class="mb-4 w-full">
        ...
        <input class="shadow appearance-none border rounded w-full py-2 px-3 bg-gray-100 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="contact" name="contact" type="text" value="{{ $user->contact }}" placeholder="{{ __('Enter Contact') }}">
      </div>
      ...
      <div class="mb-4 mx-2 w-full">
        ...
        <input class="shadow appearance-none border rounded w-full py-2 px-3 bg-gray-100 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="store_id" name="store_id" type="number" min="1" value="{{ $user->store_id }}" placeholder="{{ __('Enter Store ID') }}">
      </div>
      ...
      <div class="mb-4 mx-2 w-full">
        ...
        <input class="shadow appearance-none border rounded w-full py-2 px-3 bg-gray-100 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="store_name" name="store_name" type="text" value="{{ $user->store_name }}" placeholder="{{ __('Enter Store Name') }}">
      </div>
      ...
      <div class="mb-4 mx-2 w-full">
        ...
        <input class="shadow appearance-none border rounded w-full py-2 px-3 bg-gray-100 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="store_address" name="store_address" type="text" value="{{ $user->store_address }}" placeholder="{{ __('Enter Store Address') }}">
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="relative">
          <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" name="role" id="role">
            ...
            <option value="{{ $role->id }}" @selected(in_array($role->name, $userRoles))>{{ $role->name }}</option>
            @empty
            @endforelse
          </select>
      ...
      <div class="mb-4">
        ...
        <div class="relative">
          <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" name="language" id="language">
            ...
            <option value="{{ $langLocale }}" @selected($langLocale==$user->lang)>{{ $langName }}</option>
            @empty
      ...
      <div class="flex items-center my-2 border-b border-black">
        ...
        <div class="sm:w-3/4 p-2 flex flex-wrap flex-col sm:flex-row">
          ...
          <div class="sm:w-1/4 items-center flex">
            <input class="permissions {{ $permission->uses }}" type="checkbox" name="permission[{{ $permission->name }}]" value="{{ $permission->name }}" id="{{ $permission->name }}" data-po-uses="{{ $permission->uses }}" data-po-short=".{{ $permission->short }}" @checked(in_array($permission->name, $userPermissions))>
